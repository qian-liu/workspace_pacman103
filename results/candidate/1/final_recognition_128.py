...
runtime = 60000 # just for gesture single
source_window = 10000
overlap_window = 5000
num_per_core = 256
...
templates_file = 'template_128.mat'
#template_scale = 0.2
template_scale = [0.16, 0.2, 0.2, 0.23, 0.25]
recognition_pops = []
recognition_size = 16
...
pool_integrate[0].record()
print 'Simulation started... on: {}\n'.format(dt.datetime.now())
p.run(runtime)

# write spikes out to files
filename = 'results/00/integrate.spikes' 
pool_integrate[0].printSpikes(filename)
for iTemplate in range (0, num_templates):
    filename = 'results/00/recog_%d.spikes' %(iTemplate)
