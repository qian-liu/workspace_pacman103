...
spikeTrains, source_Num = load_inputSpikes5('recorded_data_for_spinnaker/integrate_32_all.mat', 'integrate', integrate_size, window_time)
...
for source_count in range (7, source_Num):
    ...
    pool_integrate[source_count-7].record()
for iTemplate in range (0, num_templates):
    template = load_template(iTemplate, templates_names, templates_file ) # lading tempaltes
    #template_weight = [[template_scale * x for x in y] for y in template] # play with weight_scale to move from ANN simulation to Spiking Neural Networks
    ...
    for source_count in range (7, source_Num):
        p.Projection(pool_integrate[source_count-7], recognition_pops[iTemplate], p.FromListConnector(exci_list), target='excitatory')
        p.Projection(pool_integrate[source_count-7], recognition_pops[iTemplate], p.FromListConnector(inhi_list), target='inhibitory')
...
for source_count in range (7, source_Num):
    filename = 'results/322/integrate_%d.spikes' %(source_count+1)
    pool_integrate[source_count-7].printSpikes(filename)
for iTemplate in range (0, num_templates):
    filename = 'results/322/recog_%d.spikes' %(iTemplate)
